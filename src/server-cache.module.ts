...
export function injectCacheFactory(
      ...
      .subscribe(() => {
        try {
          ...
          if (document.documentElement) {
            ...
            html = Array.from(document.children || document.childNodes).find(
              (child: any) => (child.name || child.localName) === "html"
            );
          }
          const head = Array.from(html.children || html.childNodes).find(
            (child: any) => (child.name || child.localName) === "head"
          );
...
export class ServerCacheModule {
  static forRoot(
    setupOptions: { appCacheKey?: string } = { appCacheKey: "__APP_CACHE__" }
  ): ModuleWithProviders {
    return {
